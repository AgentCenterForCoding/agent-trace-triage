...
agent-trace-triage/
├── backend/
│   ├── app.py              # FastAPI 入口（含 /api/llm/test-connection）
│   ├── models.py           # 数据模型（TriageSource, OwnerTeam.USER_INTERACTION）
│   ├── trace_parser.py     # OTLP 解析（双命名约定兼容）
│   ├── triage_engine.py    # 定界引擎（L1 规则 + L2 LLM 集成）
│   ├── router.py           # 置信度路由器
│   ├── llm_skill.py        # L2 LLM 推理模块
│   ├── rules.yaml          # 规则配置
│   ├── sample_traces/      # 样本 Trace（v1: 4_1-4_17, v2: c1-c10）
│   └── tests/
...
├── frontend/               # Web UI（LLM 配置弹窗 + Source 列 + 连通性测试）
├── docs/
│   ├── architecture.md     # 本文档
│   └── opencode-trace-spec.md # OpenCode Agent Trace 规范
└── openspec/
    ├── specs/              # 归档的规范
    └── changes/            # 提案（含 l2-llm-skill）
```
